...
export async function GET(
  ...
  try {
    ...
    const { data: link, error } = await supabase
      .from('links')
      .select('id, destination_url, alias, is_active, expires_at, password_hash, workspace_id, is_guest, total_clicks')
    ...
    if (isPromoRedirect && promoUrlId) {
      ...
      if (promoUrl?.url) {
        ...
        promoRedirect.searchParams.set('url', link.destination_url);
