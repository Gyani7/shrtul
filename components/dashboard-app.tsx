...
function LinkRow({
  ...
  const domain = getDomainFromUrl(link.destination_url);
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-start gap-3 min-w-0 flex-1">
            ...
            <img
              src={getFaviconUrl(link.destination_url)}
              alt=""
...
function CreateLinkDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (v: boolean) => void; onCreated: () => void }) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const ws = wsData?.[0];
      if (!ws) {
        const { data: newWsId, error: wsError } = await supabase.rpc('ensure_user_workspace');
        if (wsError || !newWsId) {
          setError('Could not create workspace: ' + (wsError?.message || 'unknown'));
          setLoading(false);
          return;
        }
        ...
        if (password) {
          const { data: hashData, error: hashError } = await supabase.rpc('hash_password', { pw: password });
          if (hashError) { setError('Failed to set password'); setLoading(false); return; }
          insertData.password_hash = hashData;
        }
        if (expiresAt) { insertData.expires_at = new Date(expiresAt).toISOString(); }
        if (!insertData.destination_url || typeof insertData.destination_url !== 'string' || !insertData.destination_url.trim()) {
          console.error('[dashboard] CRITICAL: destination_url empty before insert', { url, normalized });
          setError('Failed to create link: destination URL is missing'); setLoading(false); return;
        }
        console.log('[dashboard] insert (new ws):', { alias: insertData.alias, url_len: (insertData.destination_url as string).length, has_password: !!insertData.password_hash, has_expiry: !!insertData.expires_at });
      ...
      console.log('[dashboard] insert (existing ws):', { alias: insertData.alias, url_len: (insertData.destination_url as string).length, has_password: !!insertData.password_hash, has_expiry: !!insertData.expires_at });
      const { error: insertError } = await supabase.from('links').insert(insertData);
      if (insertError) {
        if (insertError.message.includes('duplicate')) {
          setError('That alias is already taken');
